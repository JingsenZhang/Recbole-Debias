...
class DICESampler(AbstractSampler):
    ...
    def _pop_sampling(self, sample_num, item_ids_repeat):
        ...
        for count, idx in enumerate(random_index_list):
            if self.prob[keys[idx]] < self.prob[item_ids_repeat[count]]:
                pop_mask_list.append(False)
            else:
                pop_mask_list.append(True)
            final_random_list.append(keys[idx])

        return np.array(final_random_list), np.array(pop_mask_list)
